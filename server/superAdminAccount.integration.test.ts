...
describe("configured super-admin account readiness", () => {
  it("maps the server-configured admin identity to one existing Peerlock account", async () => {
    ...
    const [account] = await db!.select({ id: peerlockAccounts.id, emailVerifiedAt: peerlockAccounts.emailVerifiedAt, avatarKey: peerlockAccounts.avatarKey }).from(peerlockAccounts).where(eq(peerlockAccounts.email, email)).limit(1);
