...
export function WorkspaceProvider({ children }: { children: React.ReactNode }) {
  ...
  const openRoom = async (roomCode: string, roomSecret: string) => { const found = documents.find(item => item.roomCode === roomCode && item.roomSecret === roomSecret); if (found) return found; const document = await createDocument(`Room ${roomCode}`); const room = { ...document, roomCode, roomSecret }; await save(room); return room; };
  const value = useMemo(() => ({ profile, documents, loading, setProfile, createDocument, updateDocument, removeDocument, openRoom }), [profile, documents, loading]);
