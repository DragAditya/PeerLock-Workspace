...
type Props = { document: Y.Doc; fragment: Y.XmlFragment; provider: WebrtcProvider | null; name: string; color: string; onTextChange: (text: string) => void };
const controls = [
...
export function RichEditor({ document, fragment, provider, name, color, onTextChange }: Props) {
  const extensions: any[] = [StarterKit.configure({ undoRedo: false, codeBlock: false }), CodeBlockLowlight.configure({ lowlight }), Collaboration.configure({ document, fragment }), Placeholder.configure({ placeholder: "Write something worth keeping private…\n\nUse Markdown input: # heading, - list, > quote, or ``` for code." })];
  if (provider) extensions.push(CollaborationCaret.configure({ provider, user: { name, color } }));
  const editor = useEditor({ extensions, editorProps: { attributes: { class: "rich-editor" } } }, [document, fragment, provider, name, color]);
