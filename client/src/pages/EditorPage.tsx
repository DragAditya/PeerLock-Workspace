...
import { Check, ChevronLeft, Copy, Download, FileText, Globe2, Loader2, Pencil, ShieldCheck, Sparkles, Users, WifiOff, Wifi, WandSparkles } from "lucide-react";
...
export default function EditorPage() {
  ...
  return (
    <AppShell>
          ...
          <div className="min-w-0 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/[0.08] bg-[#111722]/68 px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={autoFormat} className="flex h-8 items-center gap-1.5 rounded-lg border border-[#F4C477]/30 bg-[#F4C477]/[0.08] px-2.5 text-[11px] font-semibold text-[#F7D79D] hover:bg-[#F4C477]/[0.16]"><WandSparkles className="h-3.5 w-3.5" />{formatted ? "Formatted" : "Auto-format"}</button><button onClick={() => setAiDialogOpen(true)} className="flex h-8 items-center gap-1.5 rounded-lg border border-[#A99FF7]/35 bg-[#A99FF7]/[0.12] px-2.5 text-[11px] font-semibold text-[#D7D0FF] hover:bg-[#A99FF7]/[0.2]"><Sparkles className="h-3.5 w-3.5" />AI format</button>
      ...
      <AiFormatDialog open={aiDialogOpen} onOpenChange={setAiDialogOpen} documentText={editorRef.current?.getText() ?? ""} onApply={applyAiFormatting} onUseLocalFallback={autoFormat} />
    </AppShell>
  );
}
